...
void parseConfigFile() {
    ...
    static const char *dirthud_dir = "/DiRTHud/";

    std::string env_data = get_data_dir();
    std::string env_config = get_config_dir();

    if (!env_data.empty())
        paths.push_back(env_data + dirthud_dir + "DiRTHud.conf");

    if (!env_config.empty())
        paths.push_back(env_config + dirthud_dir + "DiRTHud.conf");
    ...
    if (!exe_path.empty() && n != std::string::npos && n < exe_path.size() - 1) {
        ...
        if (!env_config.empty())
            paths.push_back(env_config + dirthud_dir + basename + ".conf");

        // in executable's folder though not much sense in /usr/bin/
        paths.push_back(exe_path.substr(0, n) + "/DiRTHud.conf");
        ...
        if (!env_config.empty() && (basename == "wine-preloader" || basename == "wine64-preloader")) {
            ...
            while (std::getline(stream, line, '\0'))
            {
                    ...
                    && n < line.size() - 1) // have at least one character
                {
                    ...
                    if (dot < n)
                        dot = line.size();
                    paths.push_back(env_config + dirthud_dir + "wine-" + line.substr(n + 1, dot - n - 1) + ".conf");
                    break;
                }
            }
        }
